...
public class Ex4_ServerChat extends JFrame implements Runnable{
	...
	public void run() {
		...
		try {
			...
			{
				//대기중이다가
				//접속요청하는 클라이언트 허용, 허용이 되면 Socket 이 만들어진다
				Socket socket=serverSocket.accept();
				//허용된 클라이언트들을 백터에 추가한다
	...
	class ClientMember extends Thread
	{
		...
		public void run() {
			...
			//언체 클라이언트들이 메세지를 보낼지 모르므로 while문을 열어놓는다
			while(true)
			{
				//클라이언트들이 보낸 메세지를 읽는다
				try {
					String message=br.readLine();
					//확인용:클라이언트가 보낸 메서지 출력하기
					area.append(message+"\n");
					...
					case "1": {
						...
						for(ClientMember men:listMember)
						{
							men.pw.println(this.nickName+">> 님이 입장하였습니다\n");
							men.pw.flush();//이때 비로소 전송이 된다
						}
						break;
					}
						case "2":
							//보낸 메세지가 arr[1]에 들어잇다
							//접속한 모든 클라이언트들한테 누가 어떤 메세지를 보냇는지 알린다
							for(ClientMember men:listMember)
							{
								men.pw.println(this.nickName+">>"+arr[1]+"\n");
								men.pw.flush();//이때 비로소 전송이 된다
							}
